...
class User
{
    ...
    protected function setPassword($password) { $this->password = password_hash($password, PASSWORD_BCRYPT); }
    ...
    public static function create(array $data)
    {
        try {
            ...
            if (!Role::find($data['role_id'])) {
                throw new PDOException("Role ID invalide : {$data['role_id']}");
            }
            $stmt = $pdo->prepare('
    ...
    public static function getByAgent($agentId, $agencyId, $search = '', $limit = 10, $offset = 0, $roles = [])
    {
        try {
            ...
            $query = "
                ...
                WHERE u.is_deleted = 0 
                AND u.agency_id = ?
                AND (o.user_id IS NOT NULL OR t.user_id IS NOT NULL OR b.user_id IS NOT NULL)
                AND (u.username LIKE ? OR u.email LIKE ? OR u.first_name LIKE ? OR u.last_name LIKE ?)
            ";
            $params = [$agencyId, "%{$search}%", "%{$search}%", "%{$search}%", "%{$search}%"];
            if (!empty($roles)) {
