            ...
            <div class="bg-white rounded-lg shadow-sm p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" id="name" name="name" value="<?= htmlspecialchars($_POST['name'] ?? '') ?>" required minlength="3" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-construction-yellow" placeholder="Nom du bâtiment">
                    ...
                    <div>
                        ...
                        <select id="agency_id" name="agency_id" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-construction-yellow bg-white">
                            ...
                            <?php foreach ($agencies as $agency): ?>
                                <option value="<?= $agency->getId() ?>" <?= ($_POST['agency_id'] ?? '') == $agency->getId() ? 'selected' : '' ?>>
                    ...
                    <div>
                        ...
                        <select id="owner_id" name="owner_id" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-construction-yellow bg-white">
                            <option value="">Sélectionnez un propriétaire</option>
                            <?php foreach ($owners as $owner): ?>
                                <?php $user = $owner->user(); ?>
                                <option value="<?= $owner->getId() ?>"><?= htmlspecialchars($user ? $user->getFirstName() . ' ' . $user->getLastName() : 'Inconnu') ?></option>
                            <?php endforeach; ?>
                        </select>
                        <div class="error-message text-red-500 text-sm mt-1 hidden"></div>
                    </div>
                    <div>
                        ...
                        <input type="text" id="city" name="city" value="<?= htmlspecialchars($_POST['city'] ?? '') ?>" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-construction-yellow" placeholder="Ville">
                        <div class="error-message text-red-500 text-sm mt-1 hidden"></div>
                    </div>
                    <div>
                        <label for="neighborhood" class="block text-sm font-medium text-gray-700 mb-1">Quartier</label>
                        <input type="text" id="neighborhood" name="neighborhood" value="<?= htmlspecialchars($_POST['neighborhood'] ?? '') ?>" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-construction-yellow" placeholder="Quartier">
                    </div>
                    <div>
                        <label for="country" class="block text-sm font-medium text-gray-700 mb-1">Pays <span class="text-red-500">*</span></label>
                        <input type="text" id="country" name="country" value="<?= htmlspecialchars($_POST['country'] ?? '') ?>" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-construction-yellow" placeholder="Pays">
                        <div class="error-message text-red-500 text-sm mt-1 hidden"></div>
                    </div>
                    <div>
                        <label for="floors" class="block text-sm font-medium text-gray-700 mb-1">Nombre d’étages <span class="text-red-500">*</span></label>
                        <input type="number" id="floors" name="floors" value="<?= htmlspecialchars($_POST['floors'] ?? '0') ?>" required min="0" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-construction-yellow" placeholder="0">
                        <div class="error-message text-red-500 text-sm mt-1 hidden"></div>
                    </div>
                    <div>
                        ...
                        <input type="number" id="apartment_count" name="apartment_count" value="<?= htmlspecialchars($_POST['apartment_count'] ?? '0') ?>" required min="0" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-construction-yellow" placeholder="0">
                        <div class="error-message text-red-500 text-sm mt-1 hidden"></div>
                    </div>
                    <div>
                        <label for="land_area" class="block text-sm font-medium text-gray-700 mb-1">Superficie du terrain (m²)</label>
                        <input type="number" id="land_area" name="land_area" step="0.01" value="<?= htmlspecialchars($_POST['land_area'] ?? '') ?>" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-construction-yellow" placeholder="Superficie en m²">
                        <div class="error-message text-red-500 text-sm mt-1 hidden"></div>
                    </div>
                    <div>
                        ...
                        <select id="parking" name="parking" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-construction-yellow bg-white">
                            ...
                            <option value="aucun" <?= ($_POST['parking'] ?? '') === 'aucun' ? 'selected' : '' ?>>Aucun</option>
                            <option value="souterrain" <?= ($_POST['parking'] ?? '') === 'souterrain' ? 'selected' : '' ?>>Souterrain</option>
                            <option value="exterieur" <?= ($_POST['parking'] ?? '') === 'exterieur' ? 'selected' : '' ?>>Extérieur</option>
                            <option value="couvert" <?= ($_POST['parking'] ?? '') === 'couvert' ? 'selected' : '' ?>>Couvert</option>
                        </select>
                        <div class="error-message text-red-500 text-sm mt-1 hidden"></div>
                    </div>
                    <div>
                        ...
                        <select id="type_id" name="type_id" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-construction-yellow bg-white">
                            ...
                            <?php foreach ($buildingTypes as $type): ?>
                                <option value="<?= $type->getId() ?>" <?= ($_POST['type_id'] ?? '') == $type->getId() ? 'selected' : '' ?>>
                    ...
                    <div>
                        ...
                        <input type="number" id="year_built" name="year_built" value="<?= htmlspecialchars($_POST['year_built'] ?? '') ?>" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-construction-yellow" placeholder="Année">
                        <div class="error-message text-red-500 text-sm mt-1 hidden"></div>
                    </div>
                    <div>
                        ...
                        <select id="status" name="status" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-construction-yellow bg-white">
                            ...
                            <option value="disponible" <?= ($_POST['status'] ?? '') === 'disponible' ? 'selected' : '' ?>>Disponible</option>
                            <option value="vendu" <?= ($_POST['status'] ?? '') === 'vendu' ? 'selected' : '' ?>>Vendu</option>
                            <option value="en_construction" <?= ($_POST['status'] ?? '') === 'en_construction' ? 'selected' : '' ?>>En construction</option>
                            <option value="en_renovation" <?= ($_POST['status'] ?? '') === 'en_renovation' ? 'selected' : '' ?>>En rénovation</option>
                        </select>
                        <div class="error-message text-red-500 text-sm mt-1 hidden"></div>
                    </div>
                    <div>
                        <label for="price" class="block text-sm font-medium text-gray-700 mb-1">Prix (€)</label>
                        <input type="number" id="price" name="price" step="0.01" value="<?= htmlspecialchars($_POST['price'] ?? '') ?>" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-construction-yellow" placeholder="Prix en €">
                        <div class="error-message text-red-500 text-sm mt-1 hidden"></div>
                    </div>
                    <div>
                        <label for="image" class="block text-sm font-medium text-gray-700 mb-1">Image du bâtiment</label>
                        <input type="file" id="image" name="image" accept="image/jpeg,image/png,image/gif" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-construction-yellow">
                        <div class="error-message text-red-500 text-sm mt-1 hidden"></div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow-sm p-6">
                <div class="flex justify-end space-x-4">
                    ...
                    <button type="submit" class="inline-flex items-center px-6 py-2 border border-transparent rounded-lg text-sm font-medium text-construction-black bg-construction-yellow hover:bg-yellow-600 focus:outline-none focus:ring-2 focus:ring-construction-yellow">
                        <i class="fas fa-building mr-2"></i>Créer le bâtiment
                    </button>
                </div>
            </div>
        </form>
