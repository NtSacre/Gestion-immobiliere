...
<div class="min-h-screen bg-gray-50 py-6">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-3">
                    ...
                    <h1 class="text-2xl font-bold text-construction-black">Modifier le paiement</h1>
                </div>
        ...
        <form id="payment-form" method="POST" action="/payments/update/<?= htmlspecialchars($payment->getId()) ?>" class="space-y-6" novalidate>
            <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($csrf_token) ?>">
            <input type="hidden" name="id" value="<?= htmlspecialchars($payment->getId()) ?>">
                            ...
                            >
                                ...
                                <?php foreach ($leases as $lease): ?>
                                    <option 
                                        ...
                                        <?= isset($form_data['lease_id']) && $form_data['lease_id'] == $lease->getId() ? 'selected' : ($payment->getLeaseId() == $lease->getId() ? 'selected' : '') ?>
                                    >
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input
                                ...
                                id="payment_date"
                                name="payment_date"
                                value="<?= isset($form_data['payment_date']) ? htmlspecialchars($form_data['payment_date']) : htmlspecialchars($payment->getPaymentDate()) ?>"
                                required
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input
                                ...
                                step="0.01"
                                min="0"
                                value="<?= isset($form_data['amount']) ? htmlspecialchars($form_data['amount']) : htmlspecialchars($payment->getAmount()) ?>"
                                required
                                class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-construction-yellow focus:border-transparent"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            >
                                ...
                                <option value="loyer" <?= isset($form_data['type']) && $form_data['type'] === 'loyer' ? 'selected' : ($payment->getType() === 'loyer' ? 'selected' : '') ?>>Loyer</option>
                                <option value="charges" <?= isset($form_data['type']) && $form_data['type'] === 'charges' ? 'selected' : ($payment->getType() === 'charges' ? 'selected' : '') ?>>Charges</option>
                                <option value="depot" <?= isset($form_data['type']) && $form_data['type'] === 'depot' ? 'selected' : ($payment->getType() === 'depot' ? 'selected' : '') ?>>Dépôt</option>
                            </select>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            >
                                ...
                                <option value="cash" <?= isset($form_data['mode']) && $form_data['mode'] === 'cash' ? 'selected' : ($payment->getMode() === 'cash' ? 'selected' : '') ?>>Espèces</option>
                                <option value="mobile" <?= isset($form_data['mode']) && $form_data['mode'] === 'mobile' ? 'selected' : ($payment->getMode() === 'mobile' ? 'selected' : '') ?>> Paiement mobile</option>
                                <option value="carte" <?= isset($form_data['mode']) && $form_data['mode'] === 'carte' ? 'selected' : ($payment->getMode() === 'carte' ? 'selected' : '') ?>>Carte</option>
                            </select>
                            <i class="fas fa-money-bill-wave absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400"></i>
                        </div>
                        <div class="error-message text-red-500 text-sm mt-1 hidden"></div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            >
                                ...
                                <option value="paid" <?= isset($form_data['status']) && $form_data['status'] === 'paid' ? 'selected' : ($payment->getStatus() === 'paid' ? 'selected' : '') ?>>Payé</option>
                                <option value="pending" <?= isset($form_data['status']) && $form_data['status'] === 'pending' ? 'selected' : ($payment->getStatus() === 'pending' ? 'selected' : '') ?>>En attente</option>
                            </select>
                                ...
                                >
                                    ...
                                    <?php foreach ($agencies as $agency): ?>
                                        <option 
                                            ...
                                            <?= isset($form_data['agency_id']) && $form_data['agency_id'] == $agency->getId() ? 'selected' : ($payment->getAgencyId() == $agency->getId() ? 'selected' : '') ?>
                                        >
                ...
                >
                    ...
                    <?php foreach ($agents as $agent): ?>
                        <option 
                            ...
                            <?= isset($form_data['agent_id']) && $form_data['agent_id'] == $agent->getId() ? 'selected' : ($payment->getAgentId() == $agent->getId() ? 'selected' : '') ?>
                        >
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateAmount() {
        ...
        if (selectedOption && type) {
            ...
            if (!amountInput.value || !form_data.amount) {
    ...
    ['lease_id', 'payment_date', 'amount', 'type', 'mode', 'status', 'agency_id'].forEach(name => {
