...
<div class="min-h-screen bg-gray-50 py-6">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-3">
                    ...
                    <h1 class="text-2xl font-bold text-construction-black">Modifier le bail</h1>
                </div>
        ...
        <form id="lease-form" method="POST" action="/leases/update/<?= htmlspecialchars($lease->getId()) ?>" class="space-y-6" novalidate>
            <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($csrf_token) ?>">
            <input type="hidden" name="id" value="<?= htmlspecialchars($lease->getId()) ?>">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            >
                                ...
                                <?php foreach ($apartments as $apartment): ?>
                                    <option 
                                        ...
                                        <?= isset($form_data['apartment_id']) && $form_data['apartment_id'] == $apartment->getId() ? 'selected' : ($lease->getApartmentId() == $apartment->getId() ? 'selected' : '') ?>
                                    >
                            ...
                            >
                                ...
                                <?php foreach ($tenants as $tenant): ?>
                                    <option 
                                        ...
                                        <?= isset($form_data['tenant_id']) && $form_data['tenant_id'] == $tenant->getId() ? 'selected' : ($lease->getTenantId() == $tenant->getId() ? 'selected' : '') ?>
                                    >
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input
                                ...
                                id="start_date"
                                name="start_date"
                                value="<?= isset($form_data['start_date']) ? htmlspecialchars($form_data['start_date']) : htmlspecialchars($lease->getStartDate()) ?>"
                                required
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input
                                ...
                                id="end_date"
                                name="end_date"
                                value="<?= isset($form_data['end_date']) ? htmlspecialchars($form_data['end_date']) : htmlspecialchars($lease->getEndDate()) ?>"
                                class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-construction-yellow focus:border-transparent"
                            >
                            <i class="fas fa-calendar-alt absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400"></i>
                        </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input
                                ...
                                step="0.01"
                                min="0"
                                value="<?= isset($form_data['rent_amount']) ? htmlspecialchars($form_data['rent_amount']) : htmlspecialchars($lease->getRentAmount()) ?>"
                                required
                                class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-construction-yellow focus:border-transparent"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input
                                ...
                                step="0.01"
                                min="0"
                                value="<?= isset($form_data['charges_amount']) ? htmlspecialchars($form_data['charges_amount']) : htmlspecialchars($lease->getChargesAmount()) ?>"
                                required
                                class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-construction-yellow focus:border-transparent"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input
                                ...
                                step="0.01"
                                min="0"
                                value="<?= isset($form_data['deposit_amount']) ? htmlspecialchars($form_data['deposit_amount']) : htmlspecialchars($lease->getDepositAmount()) ?>"
                                required
                                class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-construction-yellow focus:border-transparent"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            >
                                ...
                                <option value="monthly" <?= isset($form_data['payment_frequency']) && $form_data['payment_frequency'] === 'monthly' ? 'selected' : ($lease->getPaymentFrequency() === 'monthly' ? 'selected' : '') ?>>Mensuel</option>
                                <option value="quarterly" <?= isset($form_data['payment_frequency']) && $form_data['payment_frequency'] === 'quarterly' ? 'selected' : ($lease->getPaymentFrequency() === 'quarterly' ? 'selected' : '') ?>>Trimestriel</option>
                            </select>
                    ...
                    <div class="flex items-center">
                        <input
                            ...
                            name="is_active"
                            value="1"
                            <?= isset($form_data['is_active']) && $form_data['is_active'] ? 'checked' : ($lease->getIsActive() ? 'checked' : '') ?>
                                ...
                                >
                                    ...
                                    <?php foreach ($agencies as $agency): ?>
                                        <option 
                                            ...
                                            <?= isset($form_data['agency_id']) && $form_data['agency_id'] == $agency->getId() ? 'selected' : ($lease->getAgencyId() == $agency->getId() ? 'selected' : '') ?>
                                        >
                                    ...
                                    >
                                        ...
                                        <?php foreach ($agents as $agent): ?>
                                            <option 
                                                ...
                                                <?= isset($form_data['agent_id']) && $form_data['agent_id'] == $agent->getId() ? 'selected' : ($lease->getAgentId() == $agent->getId() ? 'selected' : '') ?>
                                            >
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    apartmentIdInput.addEventListener('change', function() {
        ...
        // Ne mettre à jour que si les champs n'ont pas de valeur personnalisée (form_data)
        if (!rentAmountInput.value || !form_data.rent_amount) {
            rentAmountInput.value = rentAmount || '';
        }
        if (!chargesAmountInput.value || !form_data.charges_amount) {
    ...
    // Déclencher le pré-remplissage si un appartement est déjà sélectionné
